...
function canLoadAdapterCore() {
  ...
  if (process.env.IOBROKER_DATA_DIR || process.env.IOBROKER_HOST) {
    return true;
  }

  const controllerPackageNames = ['iobroker.js-controller', 'ioBroker.js-controller', 'js-controller', '@iobroker/js-controller'];
  for (const packageName of controllerPackageNames) {
    try {
