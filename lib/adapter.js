...
function canLoadAdapterCore() {
  ...
  return Boolean(resolveIoBrokerConfigPath());
}

function resolveAdapterBase() {
  if (!canLoadAdapterCore()) {
    return AdapterStub;
  }
...
const AdapterBase = resolveAdapterBase();
  ...
  async runBridge(action, payload) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this._debugEnabled || this.config?.debug) {
        ...
        this.log.debug(`Bridge payload: ${JSON.stringify(debugContext)}`);
      }
      let stdout = '';
      let stderr = '';
      ...
      child.on('close', (code) => {
        if (code !== 0) {
          ...
          this._lastLog = message;
          if (this._debugEnabled || this.config?.debug) {
            this.log.debug(`Bridge failure context: ${JSON.stringify(debugContext)}`);
          }
