...
const TYPED_COMMANDS = {
  ...
  climateStart: {
    command: 'start',
    serviceId: 'ZAF',
    setting: PARAMS([['AC', 'true'], ['AC.temp', '20.0'], ['AC.duration', '15']]),
  },
...
function resolveIoBrokerConfigPath() {
  const envCandidates = [];
  if (process.env.IOBROKER_DATA_DIR) {
    envCandidates.push(process.env.IOBROKER_DATA_DIR);
  }
  if (process.env.IOB_DATA_DIR) {
    envCandidates.push(process.env.IOB_DATA_DIR);
  }
  if (process.env.IOBROKER_HOST || process.env.OBJDB_TYPE) {
    envCandidates.push(path.join(process.cwd(), 'iobroker-data'));
  }

  for (const candidateDir of envCandidates) {
    const resolvedDir = path.isAbsolute(candidateDir) ? candidateDir : path.resolve(process.cwd(), candidateDir);
    const configPath = path.join(resolvedDir, 'iobroker.json');
  ...
  const candidateConfigPaths = [
    ...
    path.join(process.cwd(), 'data', 'iobroker.json'),
    path.join(path.dirname(process.cwd()), 'iobroker-data', 'iobroker.json'),
    path.join(path.dirname(process.cwd()), 'data', 'iobroker.json'),
  ];

  return candidateConfigPaths.find((candidatePath) => fs.existsSync(candidatePath)) || null;
}
...
class ZeekrAdapter extends AdapterStub {
  ...
  async onReady() {
    ...
    this.readyPromise = (async () => {
      ...
      if (suggested === 'EU' && this.config.extractRegion === 'EM') {
        this.log.warn('Country suggests EU but extractRegion is EM — EU-Konten (com.zeekr.overseas) brauchen EU (siehe 0001 Invalid access key).');
      }
  ...
  async onMessage(obj) {
    ...
    if (obj && obj.command === 'stateChange') {
      ...
      const typedMatch = id?.match(/vehicles\.(.+)\.control\.(lock|unlock|climateStart|climateStop|chargeStart|chargeStop|windowsOpen|windowsClose|windowsVentilate|sunshadeOpen|sunshadeClose|flash|honkFlash|applyChargePlan)$/);
      const bridgeBase = () => ({
        username: this.config.username,
        password: this.config.password,
        countryCode: this.config.countryCode || 'AU',
      ...
      if (sendMatch) {
        try {
          ...
          try {
            await this.setStateChangedAsync(`${id.replace(/\.send$/, '.lastResult')}`, JSON.stringify({ ok: false, error: error.message }), true);
          } catch {
        ...
        try {
          ...
          if (typedAction === 'applyChargePlan') {
            ...
            const preset = TYPED_COMMANDS[typedAction] || { command: typedAction, serviceId: '', setting: {} };
  ...
  async maybeAutoExtractSecrets() {
    ...
    if (!payload.secretsJsonPath && (!payload.apkBasePath || !payload.apkArm64Path)) {
      const message = 'Auto-Extraktion aktiv, aber weder APK-Pfade noch Secrets-JSON gesetzt. Siehe Admin-Hilfe Schritt 3.';
    ...
    try {
      ...
      if (Array.isArray(result.warnings) && result.warnings.length) {
        await this.setStateChangedAsync('info.extractionWarnings', result.warnings.join(' | ').slice(0, 2000), true);
  ...
  async ensureBaseObjects() {
    ...
    const infoStates = [
      ...
      { id: 'info.watchdog', common: { name: 'Watchdog status', type: 'string', role: 'text', read: true, write: false } },
      ...
      { id: 'info.countryCode', common: { name: 'Country code', type: 'string', role: 'text', read: true, write: false } },
      ...
      { id: 'info.apkBasePath', common: { name: 'APK base path', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.apkArm64Path', common: { name: 'APK arm64 path', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.extractRegion', common: { name: 'Extract region', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.suggestedRegion', common: { name: 'Suggested region for country', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.lastErrorHint', common: { name: 'Last error hint (DE)', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.extractionStatus', common: { name: 'Extraction status', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.extractionWarnings', common: { name: 'Extraction warnings', type: 'string', role: 'text', read: true, write: false } },
    ];
  ...
  async _pollVehiclesInner() {
    ...
    for (const vehicle of vehicles) {
      ...
      const states = [
        ...
        { id: `${idBase}.control.lock`, value: false, common: { name: 'Lock vehicle (button)', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.unlock`, value: false, common: { name: 'Unlock vehicle (button)', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.climateStart`, value: false, common: { name: 'Start climate (button)', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.climateStop`, value: false, common: { name: 'Stop climate (button)', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.chargeStart`, value: false, common: { name: 'Start charging (RCS)', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.chargeStop`, value: false, common: { name: 'Stop charging (RCS)', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.windowsOpen`, value: false, common: { name: 'Open windows', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.windowsClose`, value: false, common: { name: 'Close windows', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.windowsVentilate`, value: false, common: { name: 'Ventilate windows', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.sunshadeOpen`, value: false, common: { name: 'Open sunshade', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.sunshadeClose`, value: false, common: { name: 'Close sunshade', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.flash`, value: false, common: { name: 'Flash blinkers', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.honkFlash`, value: false, common: { name: 'Honk and flash', type: 'boolean', role: 'button', read: true, write: true } },
        { id: `${idBase}.control.chargePlanStart`, value: '', common: { name: 'Charge plan start HH:MM', type: 'string', role: 'text', read: true, write: true } },
        { id: `${idBase}.control.chargePlanEnd`, value: '', common: { name: 'Charge plan end HH:MM', type: 'string', role: 'text', read: true, write: true } },
        { id: `${idBase}.control.applyChargePlan`, value: false, common: { name: 'Apply charge plan (button)', type: 'boolean', role: 'button', read: true, write: true } },
        ...
        { id: `${idBase}.status.chargingLimit`, value: vehicle.chargingLimit ?? null, common: { name: 'Charge limit', type: 'number', role: 'value.battery', read: true, write: false, unit: '%' } },
        { id: `${idBase}.status.tirePressureFl`, value: vehicle.tirePressureFl ?? null, common: { name: 'Tire pressure FL', type: 'number', role: 'value.pressure', read: true, write: false, unit: 'bar' } },
        { id: `${idBase}.status.tirePressureFr`, value: vehicle.tirePressureFr ?? null, common: { name: 'Tire pressure FR', type: 'number', role: 'value.pressure', read: true, write: false, unit: 'bar' } },
        { id: `${idBase}.status.tirePressureRl`, value: vehicle.tirePressureRl ?? null, common: { name: 'Tire pressure RL', type: 'number', role: 'value.pressure', read: true, write: false, unit: 'bar' } },
        { id: `${idBase}.status.tirePressureRr`, value: vehicle.tirePressureRr ?? null, common: { name: 'Tire pressure RR', type: 'number', role: 'value.pressure', read: true, write: false, unit: 'bar' } },
        { id: `${idBase}.status.latitude`, value: vehicle.latitude ?? null, common: { name: 'Latitude', type: 'number', role: 'value.gps.latitude', read: true, write: false } },
        { id: `${idBase}.status.longitude`, value: vehicle.longitude ?? null, common: { name: 'Longitude', type: 'number', role: 'value.gps.longitude', read: true, write: false } },
        { id: `${idBase}.status.battery12v`, value: vehicle.battery12v ?? null, common: { name: '12V battery', type: 'number', role: 'value.voltage', read: true, write: false, unit: 'V' } },
        { id: `${idBase}.status.lastTripDistanceKm`, value: vehicle.lastTripDistanceKm ?? null, common: { name: 'Last trip distance', type: 'number', role: 'value.distance', read: true, write: false, unit: 'km' } },
        { id: `${idBase}.status.centralLockingStatus`, value: vehicle.centralLockingStatus || '', common: { name: 'Central locking status', type: 'string', role: 'text', read: true, write: false } },
        { id: `${idBase}.status.windowPositionAvg`, value: vehicle.windowPositionAvg ?? null, common: { name: 'Window position avg', type: 'number', role: 'value', read: true, write: false, unit: '%' } },
        { id: `${idBase}.status.chargePlanRaw`, value: JSON.stringify(vehicle.chargePlan || {}), common: { name: 'Charge plan payload', type: 'string', role: 'json', read: true, write: false } },
        { id: `${idBase}.status.travelPlanRaw`, value: JSON.stringify(vehicle.travelPlan || {}), common: { name: 'Travel plan payload', type: 'string', role: 'json', read: true, write: false } },
        { id: `${idBase}.status.vtmRaw`, value: JSON.stringify(vehicle.vtmStatus || {}), common: { name: 'VTM payload', type: 'string', role: 'json', read: true, write: false } },
  ...
  getConfigHash() {
    const canonical = JSON.stringify({
      ...
      passwordHash: this.config?.password ? crypto.createHash('sha256').update(String(this.config.password)).digest('hex').slice(0, 16) : '',
      countryCode: this.config?.countryCode || 'AU',
      secretsPresent: ['hmacAccessKey', 'hmacSecretKey', 'passwordPublicKey', 'prodSecret', 'prodSecretCandidates', 'vinKey', 'vinIv'].map(
        (k) => (this.config?.[k] ? '1' : '0'),
      ).join(''),
      autoExtractSecrets: Boolean(this.config?.autoExtractSecrets),
      apkBasePath: this.config?.apkBasePath || '',
  ...
  async runPythonScript(scriptPath, scriptArgs = [], payload = {}, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = this.setTimeout(() => fail(new Error(`Python script timed out after ${timeoutMs}ms`)), timeoutMs);
      const debugRedacted = {
        scriptPath,
        pythonBinary,
        scriptArgs,
