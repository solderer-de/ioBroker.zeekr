...
class ZeekrAdapter extends AdapterStub {
  ...
  async maybeAutoExtractSecrets() {
    ...
    if (!payload.apkBasePath || !payload.apkArm64Path) {
      const message = 'Auto-extract is enabled, but no APK paths were supplied.';
    ...
    try {
      ...
      for (const field of secretFields) {
        if (secrets[field]) {
          await this.setStateChangedAsync(`info.${field}`, secrets[field], true);
        }
      }
      this._lastLog = `Extracted ${Object.keys(secrets).filter((key) => secrets[key]).length} Zeekr secrets successfully.`;
  ...
  async ensureBaseObjects() {
    ...
    const infoStates = [
      ...
      { id: 'info.hmacAccessKey', common: { name: 'HMAC access key', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.hmacSecretKey', common: { name: 'HMAC secret key', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.passwordPublicKey', common: { name: 'Password public key', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.prodSecret', common: { name: 'Production secret', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.vinKey', common: { name: 'VIN key', type: 'string', role: 'text', read: true, write: false } },
      { id: 'info.vinIv', common: { name: 'VIN IV', type: 'string', role: 'text', read: true, write: false } },
  ...
  async runPythonScript(scriptPath, scriptArgs = [], payload = {}, options = {}) {
    ...
    const bootstrapArgs = [scriptPath, ...scriptArgs, input];
    return new Promise((resolve, reject) => {
      ...
      if (this._debugEnabled || this.config?.debug) {
        ...
        this.log.debug(`Script payload: ${JSON.stringify(debugContext)}`);
      }
      let stdout = '';
      let stderr = '';
      ...
      child.on('close', (code) => {
        if (code !== 0) {
          const message = stderr || `Python script exited with ${code}`;
          this._lastError = message;
          this._lastLog = message;
          if (this._debugEnabled || this.config?.debug) {
            this.log.debug(`Python script failure context: ${JSON.stringify(debugContext)}`);
          }
        ...
        try {
          ...
          const message = `Invalid python script output: ${stdout}`;
          this._lastError = message;
          this._lastLog = message;
          reject(new Error(message));
        }
