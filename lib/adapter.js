...
class ZeekrAdapter extends AdapterBase {
  ...
  async ensureBaseObjects() {
    ...
    await this.setObjectNotExistsAsync('info.connection', {
      type: 'state',
      common: { name: 'Connection state', type: 'boolean', role: 'indicator.connected', read: true, write: false },
      native: {},
    });
  ...
  async pollVehicles() {
    ...
    const result = await this.runBridge('vehicles', {
      username: this.config.username,
      password: this.config.password,
      vehicleFilter: this.config.vehicleFilter || '',
    });
  ...
  async runBridge(action, payload) {
    ...
    return new Promise((resolve, reject) => {
      const child = spawn('python3', [scriptPath, action, input], {
