...
// Typisierte Buttons: Defaults zum Nachjustieren pro Modell/Region.
// Charge läuft über serviceId RCS (CHARGE_CONTROL_URL), Rest über REMOTECONTROL_URL.
const TYPED_COMMANDS = {
  lock: { command: 'lock', serviceId: '', setting: {} },
  unlock: { command: 'unlock', serviceId: '', setting: {} },
  climateStart: { command: 'climate_start', serviceId: '', setting: {} },
  climateStop: { command: 'climate_stop', serviceId: '', setting: {} },
...
class ZeekrAdapter extends AdapterStub {
  ...
  async onMessage(obj) {
    ...
    if (obj && obj.command === 'stateChange') {
      ...
      const typedMatch = id?.match(/vehicles\.(.+)\.control\.(lock|unlock|climateStart|climateStop|chargeStart|chargeStop|applyChargePlan)$/);
      const bridgeBase = () => ({
        username: this.config.username,
        password: this.config.password,
        countryCode: this.config.countryCode || 'AU',
